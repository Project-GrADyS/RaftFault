...
from raft_fault import RaftConfig, RaftConsensus, RaftMode
...
class RaftProtocol(IProtocol):
    def initialize(self):
        ...
        # Use the adapter to get the node ID
        self.node_id = self.adapter.get_node_id()
    ...
    def handle_telemetry(self, telemetry: Telemetry) -> None:
        ...
        if self.consensus.is_leader():
            self.adapter.paint_node("green", self.node_id)
        elif self.consensus.is_simulation_active(self.node_id):
            self.adapter.paint_node("blue", self.node_id)
        else:
            self.adapter.paint_node("red", self.node_id)
